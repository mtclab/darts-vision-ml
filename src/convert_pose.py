...
def xy_list_to_array(xy_list: list) -> np.ndarray:
    ...
    arr[:n, 2] = 2  # set to 2 for visible in pose format
    return arr
...
def save_pose_label(img_name: str, keypoints: np.ndarray, output_dir: Path):
    ...
    line = " ".join(f"{p:.6f}" if isinstance(p, float) else str(p) for p in parts) + "\n"
...
def split_and_write(img_paths, keypoints_list, output_root: Path, test_size=0.15, val_size=0.15, random_state=42):
    ...
    for split_name, indices in splits.items():
        ...
        for idx in indices:
            ...
            save_pose_label(img_name, keypoints_list[idx], split_dir)
...
def main():
    ...
    parser.add_argument("--output", default="data/processed/yolo_pose_darts", help="Output directory")
