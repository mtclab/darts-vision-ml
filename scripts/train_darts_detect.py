#!/usr/bin/env python3
"""
Train darts-detect model (board bbox + dart tip bboxes).

Alternative to darts-pose — bounding boxes only, less precise.
Use only if keypoint detection is unavailable.

Usage:
    python scripts/train_darts_detect.py --gpu 0
    python scripts/train_darts_detect.py --gpu 0,1,2,3
    python scripts/train_darts_detect.py --resume runs/darts_detect/train/weights/last.pt
"""
...
RUN_DIR = Path.cwd() / "runs" / "darts_detect"
CONFIG = "configs/dataset_detect.yaml"


def train_darts_detect(args):
...
def main():
    parser = argparse.ArgumentParser(description="Train darts-detect model (bbox only)")
